...
func (m *mgr) getByKey(ctx context.Context, key *collaboration.ShareKey) (*collaboration.Share, error) {
	...
	if err := m.db.QueryRow(query, owner, key.Ref.StorageId, key.Ref.NodeId, shareType, shareWith, uid, uid).Scan(&s.UIDOwner, &s.UIDInitiator, &s.ShareWith, &s.Prefix, &s.ItemSource, &s.ID, &s.STime, &s.Permissions, &s.ShareType); err != nil {
		if err == sql.ErrNoRows {
...
func (m *mgr) Unshare(ctx context.Context, ref *collaboration.ShareReference) error {
	...
	case ref.GetKey() != nil:
		...
		params = append(params, owner, key.Ref.StorageId, key.Ref.NodeId, shareType, shareWith, uid, uid)
	default:
...
func (m *mgr) UpdateShare(ctx context.Context, ref *collaboration.ShareReference, p *collaboration.SharePermissions) (*collaboration.Share, error) {
	...
	case ref.GetKey() != nil:
		...
		params = append(params, permissions, time.Now().Unix(), owner, owner, key.Ref.StorageId, key.Ref.NodeId, shareType, shareWith, uid, uid)
	default:
...
func (m *mgr) getReceivedByKey(ctx context.Context, key *collaboration.ShareKey) (*collaboration.ReceivedShare, error) {
	...
	shareType, shareWith := conversions.FormatGrantee(key.Grantee)
	params := []interface{}{uid, conversions.FormatUserID(key.Owner), key.Ref.StorageId, key.Ref.NodeId, shareType, shareWith, shareWith}
	for _, v := range user.Groups {
		params = append(params, v)
	}
