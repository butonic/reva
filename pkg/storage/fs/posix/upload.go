...
//2021-02-05T21:47:41Z ERR home/vscode/repositories/reva/internal/grpc/services/storageprovider/storageprovider.go:347 > error getting upload id: path:"/home/maxresdefault.jpg"  error="open /var/tmp/ocis/storage/posix/uploads/70ff2e52-cc91-4354-bf83-390c8c5b3fc3: no such file or directory
...
func (fs *posixfs) Upload(ctx context.Context, ref *provider.Reference, r io.ReadCloser) (err error) {
	...
	if err != nil {
		...
		if upload, err = fs.GetUpload(ctx, uploadIDs["simple"]); err != nil {
			return errors.Wrap(err, "posixfs: error retrieving upload")
		}
	}
	...
	if err != nil {
		return errors.Wrap(err, "posixfs: error checking path")
	}
	if ok {
		...
		if err != nil {
			return errors.Wrap(err, "posixfs: error opening assembled file")
		}
		defer fd.Close()
	...
	if _, err := uploadInfo.WriteChunk(ctx, 0, r); err != nil {
		return errors.Wrap(err, "posixfs: error writing to binary file")
	}
...
func (fs *posixfs) InitiateUpload(ctx context.Context, ref *provider.Reference, uploadLength int64, metadata map[string]string) (map[string]string, error) {
	...
	log.Debug().Interface("info", info).Interface("node", n).Interface("metadata", metadata).Msg("posixfs: resolved filename")
...
func (fs *posixfs) NewUpload(ctx context.Context, info tusd.FileInfo) (upload tusd.Upload, err error) {
	...
	log := appctx.GetLogger(ctx)
	log.Debug().Interface("info", info).Msg("posixfs: NewUpload")

	fn := info.MetaData["filename"]
	if fn == "" {
		return nil, errors.New("posixfs: missing filename in metadata")
	}
	info.MetaData["filename"] = filepath.Clean(info.MetaData["filename"])

	dir := info.MetaData["dir"]
	if dir == "" {
		return nil, errors.New("posixfs: missing dir in metadata")
	}
	...
	log.Debug().Interface("info", info).Interface("node", n).Msg("posixfs: resolved filename")

	// the parent owner will become the new owner
	p, perr := n.Parent()
	if perr != nil {
		return nil, errors.Wrap(perr, "posixfs: error getting parent "+n.Dir)
	}
	...
	case !ok:
		return nil, errtypes.PermissionDenied(filepath.Join(n.Dir, n.Name))
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "posixfs: error resolving upload path")
	}
	...
	log.Debug().Interface("info", info).Msg("posixfs: built storage info")
	...
	if !info.SizeIsDeferred && info.Size == 0 {
		log.Debug().Interface("info", info).Msg("posixfs: finishing upload for empty file")
...
func (fs *posixfs) getUploadPath(ctx context.Context, uploadID string) (string, error) {
	return filepath.Join(fs.o.Root, "uploads", uploadID), nil
}

// GetUpload returns the Upload for the given upload id
func (fs *posixfs) GetUpload(ctx context.Context, id string) (tusd.Upload, error) {
...
func (upload *fileUpload) WriteChunk(ctx context.Context, offset int64, src io.Reader) (int64, error) {
	...
	// However, for the posix driver it's not important whether the stream has ended
...
func (upload *fileUpload) FinishUpload(ctx context.Context) (err error) {
	...
	adler32h := adler32.New()
	{
		...
		if err != nil {
			sublog.Err(err).Msg("posixfs: could not open file for checksumming")
		...
		if _, err := io.Copy(adler32h, r2); err != nil {
			sublog.Err(err).Msg("posixfs: could not copy bytes for checksumming")
		}
	}
	...
	if err = os.Remove(upload.infoPath); err != nil {
		if !os.IsNotExist(err) {
			sublog.Err(err).Msg("posixfs: could not delete upload info")
			return
		}
	}
	...
	/*if upload.info.MetaData["mtime"] != "" {
		...
		if err != nil {
			log.Err(err).Interface("info", upload.info).Msg("posixfs: could not set mtime metadata")
			return err
		}
	}*/
...
func tryWritingChecksum(log *zerolog.Logger, n *Node, algo string, h hash.Hash) {
	if err := n.SetChecksum(algo, h); err != nil {
		log.Err(err).
			...
			Msg("posixfs: could not write checksum")
...
func (upload *fileUpload) discardChunk() {
	if err := os.Remove(upload.binPath); err != nil {
		if !os.IsNotExist(err) {
			appctx.GetLogger(upload.ctx).Err(err).Interface("info", upload.info).Str("binPath", upload.binPath).Interface("info", upload.info).Msg("posixfs: could not discard chunk")
			return
		}
	}
}
...
func (fs *posixfs) AsTerminatableUpload(upload tusd.Upload) tusd.TerminatableUpload {
...
func (fs *posixfs) AsLengthDeclarableUpload(upload tusd.Upload) tusd.LengthDeclarableUpload {
	return upload.(*fileUpload)
}
...
func (fs *posixfs) AsConcatableUpload(upload tusd.Upload) tusd.ConcatableUpload {
	return upload.(*fileUpload)
}
