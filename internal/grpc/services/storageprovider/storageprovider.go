...
func (s *service) InitiateFileDownload(ctx context.Context, req *provider.InitiateFileDownloadRequest) (*provider.InitiateFileDownloadResponse, error) {
	...
	if isStorageSpaceReference(req.Ref) {
		...
		u.Path = path.Join(u.Path, "spaces", req.Ref.NodeId, req.Ref.Path) // FIXME @butonic storageid
	} else {
		...
		u.Path = path.Join(u.Path, "simple", req.Ref.Path)
	}
...
func (s *service) Delete(ctx context.Context, req *provider.DeleteRequest) (*provider.DeleteResponse, error) {
	if req.Ref.GetPath() == "/" {
		return &provider.DeleteResponse{
			// TODO @butonic hm actually an empty path in the reference could still be a node based reference
...
func (s *service) CreateReference(ctx context.Context, req *provider.CreateReferenceRequest) (*provider.CreateReferenceResponse, error) {
	...
	if err := s.storage.CreateReference(ctx, req.Ref, u); err != nil {
