...
func (s *svc) statHome(ctx context.Context) (*provider.StatResponse, error) {
	...
	if etagIface, err := s.etagCache.Get(statRes.Info.Owner.OpaqueId + ":" + statRes.Info.Path); err == nil { // FIXME @butonic REFERENCE also use node fo etag cache?
		...
		if s.c.EtagCacheTTL > 0 {
			_ = s.etagCache.Set(statRes.Info.Owner.OpaqueId+":"+statRes.Info.Path, etagWithTS{statRes.Info.Etag, time.Now()}) // FIXME @butonic REFERENCE also use node fo etag cache?
		}
	}
...
func (s *svc) stat(ctx context.Context, req *provider.StatRequest) (*provider.StatResponse, error) {
	...
	// TODO why would we have more than one provider? shouldnt the reference always be good enough for a storage provider? regardless of the properties that are set
	// if the reference only has a path the storage registry will send us to the correct provider, right?
	...
	return &provider.StatResponse{
		...
		Info: &provider.ResourceInfo{
			Id: &provider.Reference{
				StorageId: "/", // TODO what would this mean?
